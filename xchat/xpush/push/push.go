...
func OfflineMsg(from, to, source, token, content string, chatID, interval int64,
	...
	if strings.ToLower(source) != "appstore" {
		...
		reqPush := &xg.ReqPush{
			PushType:     xg.PushTypeSingleDevice,
			DeviceToken:  token,
			MessageType:  xg.MessageTypeNotify,
			Message:      message,
			ExpireTime:   300,
			SendTime:     time.Now(),
			MultiPkgType: xg.MultiPkgAid,
			PushEnv:      xg.PushEnvAndroid,
			PlatformType: xg.PlatformAndroid,
			LoopTimes:    2,
			LoopInterval: 7,
			Cli:          androidClient,
		}
	...
	reqPush := &xg.ReqPush{
		PushType:     xg.PushTypeSingleDevice,
		DeviceToken:  token,
		MessageType:  xg.MessageTypeIos,
		Message:      message,
		ExpireTime:   0,
		SendTime:     time.Now(),
		MultiPkgType: xg.MultiPkgIos,
		PushEnv:      xg.PushEnv(env),
		PlatformType: xg.PlatformIos,
		LoopTimes:    2,
		LoopInterval: 7,
		Cli:          iosClient,
	}
